...
# ── Path resolution (portable, survives any CWD) ──────────────────────────────
SCRIPT_DIR = Path(__file__).resolve().parent.parent  # skill root (Auto-Redbook-Skills/)
...
async def measure_content_height(page: Page, html_content: str,
                                  timeout_ms: int = 20_000) -> int:
    """使用 Playwright 测量实际内容高度（含超时处理）"""
    try:
    ...
    try:
        async with async_playwright() as p:
            try:
                ...
                raise RuntimeError(f"Playwright chromium 启动失败: {launch_err}") from launch_err
            ...
            try:
                ...
                print(f"  ❌ 渲染失败 [{output_path}]: {render_err}")
                raise
...
async def process_and_render_cards(card_contents: List[str], output_dir: str,
                                   style_key: str,
                                   timeout_ms: int = 20_000) -> List[str]:
    """
    处理卡片内容，检测高度并自动分页（含超时与错误处理）
    返回最终生成的所有卡片内容列表
    """
    all_cards: List[str] = []
    browser = None
    ...
    try:
        async with async_playwright() as p:
            try:
                ...
                raise RuntimeError(f"Playwright chromium 启动失败 (process): {launch_err}") from launch_err
                ...
                for content in card_contents:
                    ...
                    for split_content in split_contents:
                        try:
                            ...
                            actual_height = await measure_content_height(page, temp_html, timeout_ms)
                        except Exception as meas_err:
                            print(f"  ⚠️ 高度测量异常，直接追加内容: {meas_err}")
                        ...
                        if actual_height > CARD_HEIGHT - 100:
                            ...
                            for line in lines:
                                ...
                                try:
                                    ...
                                    test_height = await measure_content_height(page, test_html, timeout_ms)
                                except Exception:
                                    test_height = CARD_HEIGHT  # 降级
        ...
        print(f"  ❌ process_and_render_cards 意外错误: {outer_err}")
        raise
    ...
    try:
        async with async_playwright() as p:
            try:
                ...
                raise RuntimeError(f"Playwright chromium 启动失败 (cards): {launch_err}") from launch_err
