...
def load_cookie() -> str:
    """从 .env 文件加载 Cookie（路径可移植，支持任意 workspace）"""
    # ── 优先级：CWD → 技能根目录 → 技能根的上一级 ─────────────────────────────
    # Path(__file__).resolve() 确保无论从哪里调用，路径都基于脚本本身的位置
    skill_root = Path(__file__).resolve().parent.parent   # Auto-Redbook-Skills/
    env_paths = [
        Path.cwd() / '.env',
        skill_root / '.env',
...
class ApiPublisher:
    ...
    def _request_with_retry(method: str, url: str, max_retries: int = 2,
        ...
        for attempt in range(1, max_retries + 2):  # e.g. 3 total attempts
            try:
                ...
                last_err = e
                print(f"  ⚠️ 请求超时 (尝试 {attempt}/{max_retries + 1}): {url}")
            except requests.exceptions.ConnectionError as e:
                last_err = e
                print(f"  ⚠️ 连接错误 (尝试 {attempt}/{max_retries + 1}): {url}")
            if attempt <= max_retries:
                time.sleep(1.5 * attempt)  # 简单退避
        raise requests.exceptions.RequestException(
            f"请求失败（{max_retries + 1} 次尝试后）: {last_err}"
        )
    ...
    def init_client(self):
        ...
        # 健康检查（含重试）
        try:
            resp = self._request_with_retry('GET', f"{self.api_url}/health",
                                             max_retries=2, timeout=8)
            if resp.status_code != 200:
                raise Exception(f"API 服务返回非 200 状态: {resp.status_code}")
